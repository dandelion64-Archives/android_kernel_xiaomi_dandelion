...
static phys_addr_t memtest_user_v2p(unsigned long va)
{
	...
	if (current == NULL) {
		pr_debug("warning: memtest_user_v2p, current is NULL!\n");
		return 0;
	}
	if (current->mm == NULL) {
		pr_debug("warning: memtest_user_v2p, current->mm is NULL!\n");
	...
	if (pgd_none(*pgd) || pgd_bad(*pgd)) {
		pr_debug("memtest_user_v2p, va=0x%lx, pgd invalid!\n", va);
		return 0;
	}

	pud = pud_offset(pgd, va);
	if (pud_none(*pud) || pud_bad(*pud)) {
		pr_debug("memtest_user_v2p, va=0x%lx, pud invalid!\n", va);
		return 0;
	}

	pmd = pmd_offset(pud, va);
	if (pmd_none(*pmd) || pmd_bad(*pmd)) {
		pr_debug("(memtest_user_v2p, va=0x%lx, pmd invalid!\n", va);
		return 0;
	}
	...
	pr_debug("memtest_user_v2p, va=0x%lx, pte invalid!\n", va);
	return 0;
}
