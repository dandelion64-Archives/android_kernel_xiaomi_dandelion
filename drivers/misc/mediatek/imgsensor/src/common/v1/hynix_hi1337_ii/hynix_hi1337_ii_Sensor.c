...
static int hi1337_vendor_id_read(int addr)
{
    ...
    pr_debug("hynix_hi1337_II  read vendor id , form 0x01 is: 0x%x\n", flag);
	return flag;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if( flag != HI1337_VENDOR_ID) {
        pr_debug("hynix_hi1337_I match vendor id fail, reead vendor id is: 0x%x,expect vendor id is 0x42 \n", flag);
        return ERROR_SENSOR_CONNECT_FAIL;
    }else{
        hi1337_fusion_id_read();
    }
    pr_debug("hynix_hi1337_II match vendor id successed, reead vendor id is: 0x%x,expect vendor id is 0x42 \n", flag);
