...
static int hi1337_vendor_id_read(int addr)
{
    ...
    pr_debug("hynix_hi1337_III  read vendor id , form 0x10 is: 0x%x\n", flag);
	return flag;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if( flag != HI1337_VENDOR_ID) {
        pr_debug("hynix_hi1337_I match vendor id fail, reead vendor id is: 0x%x,expect vendor id is 0x41 \n", flag);
        return ERROR_SENSOR_CONNECT_FAIL;
    }else{
        hi1337_fusion_id_read();
    }
    pr_debug("hynix_hi1337_II match vendor id successed, reead vendor id is: 0x%x,expect vendor id is 0x41 \n", flag);
...
{
    pr_debug("[hi1337] preview mode start\n");
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_debug("[hi1337] capture mode start\n");
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_debug("[hi1337] normal video mode start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_debug("[hi1337] hs video mode start\n");
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_debug("[hi1337] slim video mode start\n");
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_debug("[hi1337] custom1 mode start\n");
    spin_lock(&imgsensor_drv_lock);
