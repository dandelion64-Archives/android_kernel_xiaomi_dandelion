...
int sched_get_nr_running_avg(int *avg, int *iowait_avg)
{
	...
	*iowait_avg = 0;
	...
	if (clk_faulty) {
		*avg = 0;
		*iowait_avg = 0;
		pr_debug("[%s] **** CPU (0x%08x)clock may unstable !!\n",
					__func__, cpumask);
		return 0;
	}
	/* ///// */
...
int reset_heavy_task_stats(int cpu)
{
	...
	if (cpu_overutil->nr_overutil_l != 0 ||
		...
		pr_debug("[over-utiled task] %s: nr_overutil_l=%d nr_overutil_h=%d\n",
			__func__,
...
void overutil_thresh_chg_notify(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (cid == 0) {
			...
			pr_debug("%s: cid=%d is out of nr=%d\n", __func__,
				cid, cluster_nr);
...
static int init_heavy_tlb(void)
{
	if (!init_heavy) {
		...
		for_each_possible_cpu(tmp_cpu) {
			...
			pr_debug("%s: cpu=%d thresh_l=%d thresh_h=%d max_capaicy=%lu\n",
				__func__, tmp_cpu,
				cpu_overutil->overutil_thresh_l,
				cpu_overutil->overutil_thresh_h,
				(unsigned long int)
